...
class PlayerWidget extends StatefulWidget {
  const PlayerWidget({super.key, required this.playListKey});

  final GlobalKey<PlayListWidgetState> playListKey;
...
class PlayerWidgetState extends State<PlayerWidget> {
  ...
  // 事件总线订阅
  late StreamSubscription _addAndPlaySubscription;
  ...
  _initEventListener() {
    ...
    _playerStateSubscription = _player.onPlayerStateChanged.listen((event) {
      ...
      if (event == PlayerState.completed) {
        play(widget.playListKey.currentState?.next);
      }
    });
    // 进度改变
    ...
    _addAndPlaySubscription = EventBus().addAndPlay.stream.listen((event) {
      addAndPlay([event]);
    });
  }
                  ...
                  children: [
                    Expanded(
                        child: IconButton(
                            ...
                            onPressed: () {
                              play(widget.playListKey.currentState?.pre);
                            })),
                    Expanded(
                        child: IconButton(
  ...
  /// 添加至播放列表并播放最后一个
  addAndPlay(List<Music> musics) {
    widget.playListKey.currentState?.addAll(musics);
    Music? next = widget.playListKey.currentState?.next;
    if (next != null) {
      play(next);
    }
  }
  ...
  void dispose() {
    ...
    _addAndPlaySubscription.cancel();
  }
}
